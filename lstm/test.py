...
def add_created_id_result():
    return to_drive[steps][3]

def add_created_current_segment():
    return to_drive[steps][4]

def add_created_next_segment():
    return to_drive[steps+1][4]
...
df_scaled = df.copy()
df_scaled[['X-coordinate', 'Y-coordinate', 'Heading', 'Going to ID', 'Current segment', 'Next segment']] = _scaler.fit_transform(df[['X-coordinate', 'Y-coordinate', 'Heading', 'Going to ID', 'Current segment', 'Next segment']])
...
steps += 1
for i in range(500):
    df2 = pd.DataFrame(finData, columns=['X-coordinate', 'Y-coordinate', 'Heading', 'Going to ID', 'Current segment', 'Next segment'])
    df2 = df2.values
    toPredict = create_dataset(df2)
    predicted = model.predict(toPredict)
    # finData.append(to_drive[steps])
    finData.append((predicted[0][0], predicted[0][1], predicted[0][2], to_drive[steps][3], to_drive[steps][4], to_drive[steps][5]))
